...
export const ContactPanel = () => {
  ...
  const parseUserAgent = useMemo(() => {
    return (userAgent?: string) => {
      ...
      // Before deploying the service into production,
      // we SHOULD check the law as to the legitimicy
    ...
    return [
      {
        ...
        items: [
          {
            label: "Browser",
            value: `${userAgentInfo.browser}${!!userAgentInfo.browserVersion && " "}${!!userAgentInfo.browserVersion && userAgentInfo.browserVersion}`,
          },
          {
            label: "OS",
            value: `${userAgentInfo.os}${!!userAgentInfo.osVersion && " "}${!!userAgentInfo.osVersion && userAgentInfo.osVersion}`,
          },
          {
            label: "Device",
            value: `${userAgentInfo.device}${!!userAgentInfo.deviceModel && " "}${!!userAgentInfo.deviceModel && userAgentInfo.deviceModel}`,
        ...
        {contactSession.metadata && (
          ...
          >
            {accordionSections.map((section) => (
              ...
              >
                <AccordionTrigger className="flex w-full flex-1 items-start justify-between gap-4 rounded-none focus:bg-accent px-5 py-4 text-left font-medium text-sm outline-none transition-all hover:no-underline disabled:pointer-events-none disabled:opacity-50">
