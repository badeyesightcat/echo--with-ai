...
export const addFile = action({
  ...
  handler: async (ctx, args) => {
    ...
    if (!subscription || subscription.status !== "active") {
      throw new ConvexError({
        code: "BAD_REQUEST",
        message: "Subscription to Pro plan required",
      });
    }

    const { filename, category, bytes, mimeType: argsMimeType } = args;
    const mimeType = argsMimeType || guessMimeType(filename, bytes);
    ...
    const { entryId, created } = await rag.add(ctx, {
      ...
      // If not added, it will be considered global (we do not want this option)
      namespace: orgId,
      text,
      key: filename,
      title: filename,
      ...
      contentHash: await contentHashFromArrayBuffer(bytes), // To avoid re-inserting if th file content hasn't changed
    });
